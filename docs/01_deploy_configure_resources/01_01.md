...
Make sure to choose from one of the following regions for the resource group location (in alphabetical order rather than order of preference):
...
Before you can deploy the Bicep script, you need to ensure that you are logged into the Azure CLI. You can do so using `az login` on a local machine or `az login --use-device-code` in a Codespace VM.
...
The next step is to deploy the Bicep stack. You can do this either via CLI, or you can use the Visual Studio Code Bicep extension.
...
Finally, you will be prompted for a parameter file. Select **None** to continue.
